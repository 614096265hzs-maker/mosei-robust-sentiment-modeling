"""Build an auditable, approximate text-token/video-time map for attachment 4.

Matching token IDs establish token-to-word correspondence; forced alignment
provides approximate word times. It does not verify when official audio/vision
features were extracted, so those modalities remain `unavailable`.
"""
...
def main():
    ...
    ap.add_argument("--bert", type=Path, required=True)
    ...
    for number in range(1, 21):
        ...
        try:
            ...
            if any(key not in encoded for key in fields): raise ValueError("Tokenizer missing BERT input field")
            reconstructed = np.stack([encoded[key][0] for key in fields])
            if not np.array_equal(reconstructed, bert): raise ValueError("BERT token IDs/mask differ from attachment")
